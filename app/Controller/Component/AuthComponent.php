...
class AuthComponent extends Component {
	...
	public $components = array('Session', 'RequestHandler','RmCommon', 'RmUser');
	...
	public function login($user = null, $verify = false, $allowChanges = false) {
		$this->_setDefaults();
		if(!empty($this->request->data)){
			$this->request->data['User']['base_url'] = FULL_BASE_URL;
		}
	...
	public function identify($data = false, $verify = false) {
		...
		if( !empty($username) && !empty($data['User']['password']) ) {
			if($verify){
			...
			$user = $model->getData('first', array(
				'conditions' => array(
					...
					'User.group_id' => array( 6,11,19,20 ),
			...
			if(empty($user)){
				$msg = array(
					'status' 		=> 'error',
					'errormessage' 	=> 'Gagal melakukan login, email atau password Anda tidak valid' 
				);
			}

			if( !empty($user) ) {
				$user_id = $this->RmCommon->filterEmptyField( $user, 'User', 'id' );
				$model->doUpdateLastLogin($user_id);
			}
			$user['msg'] = $msg;
