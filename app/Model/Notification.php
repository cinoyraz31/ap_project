...
class Notification extends AppModel {
	...
	function getData( $find, $options = false, $elements = array() ){
        ...
        if( !empty($mine) ) {
        	$default_options['conditions']['Notification.user_id'] = $user_login_id;
        }
    ...
    function doSave( $data = NULL ){
		if( !empty($data) ) {
            ...
            $bank_id = !empty($data['Notification']['bank_id'])?$data['Notification']['bank_id']:false;
            $admin = !empty($data['Notification']['admin'])?$data['Notification']['admin']:false;

            if( !empty($bank_id) ) {
                $users = $this->User->getAdmins($bank_id);
            
            } else if( !empty($admin) ) {
                $users = $this->User->getAdmins(false, array(19, 20));
            }
