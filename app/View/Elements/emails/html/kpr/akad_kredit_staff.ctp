			...
			<td>
				<?php  
						echo $this->Rumahku->_callLbl('table', __('Tanggal'), sprintf(__(': %s'), $date));
      					...
      					echo $this->Rumahku->_callLbl('table', __('Staff / Bertemu dengan'), sprintf(__(': %s'), $staff_bank));
      					echo $this->Rumahku->_callLbl('table', __('No. Hanphone'), sprintf(__(': %s'), $staff_phone));
