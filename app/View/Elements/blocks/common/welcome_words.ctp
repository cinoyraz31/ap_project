...
<div class="greeting">
    <?php 
            ...
            switch ($action_type) {
                case 'sell':
                    echo $this->Html->tag('p', sprintf(__('Anda berada di halaman pasang iklan properti. Lengkapi data mengenai properti Anda pada kolom di bawah ini, dan ikuti tahapannya. Atau Anda juga bisa mengunggah berkas CSV/XLS di sini, untuk memasang lebih dari 1 iklan secara bersamaan. %s'), $this->Html->link(sprintf(__('Info detil %s'), $this->Rumahku->icon('angle-right')), '#', array(
                        'escape' => false,
                    ))));
                    break;
